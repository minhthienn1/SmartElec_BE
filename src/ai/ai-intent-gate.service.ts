...
export class AiIntentGateService {
  ...
  private inferOutOfScopeDeviceCategory(
    ...
    if (this.includesAnyPhrase(expandedText, ['laptop', 'may tinh xach tay'])) {
      return 'LAPTOP';
    }
  ...
  private buildDirectResponse(input: {
    ...
    if (input.intent === 'OUT_OF_SCOPE_TECHNICAL') {
      return 'Hiện tại SmartElec chủ yếu hỗ trợ thiết bị điện gia dụng. Thiết bị bạn mô tả có vẻ nằm ngoài phạm vi hỗ trợ chính, mình có thể gợi ý kiểm tra an toàn sơ bộ nếu bạn cần.';
    }
